...
<div class="header_user_info welcome_note">
	{if $is_logged}
		{l s='Welcome'  mod='blockuserinfo'} <span>{$cookie->customer_firstname} {$cookie->customer_lastname}</span>
	{else}
		{l s='Welcome'  mod='blockuserinfo'}<span>{l s='Guest' mod='blockuserinfo'}</span>
	{/if}
</div>
